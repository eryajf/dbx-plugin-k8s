...
export function useResourcesWatch<T extends ResourceType>(
  ...
  const dbxStopRef = useRef<(() => void) | null>(null)
  ...
  const disconnect = useCallback(() => {
    dbxStopRef.current?.(); dbxStopRef.current = null
  ...
  const connect = useCallback(() => {
    ...
    if (transport) {
      setError(null); setIsLoading(true); setIsConnected(false)
      dbxStopRef.current = startDBXWatch(transport.connectionId, resource, namespace, {invoke: transport.invoke, labelSelector: options?.labelSelector, fieldSelector: options?.fieldSelector}, event => {
        setIsLoading(false); setIsConnected(true)
        if (event.type === 'RESET') {setData([] as unknown as ResourcesItems<T>); return}
        if (!['ADDED', 'MODIFIED', 'DELETED'].includes(event.type)) return
        setData(previous => {
          const key = (item: {metadata?: {name?: string; namespace?: string}}) => `${item.metadata?.namespace || ''}/${item.metadata?.name || ''}`
          const list = (previous || []).filter(item => key(item) !== key(event.object))
          if (event.type !== 'DELETED') list.push(event.object as unknown as ResourceTypeMap[T])
          return list as ResourcesItems<T>
        })
      }, reason => {setIsLoading(false); setIsConnected(false); setError(reason instanceof Error ? reason : new Error(String(reason)))})
      return
    }
    const url = buildUrl()
    setError(null)
  ...
  }, [buildUrl, disconnect, options?.enabled, options?.labelSelector, options?.fieldSelector, resource, namespace])
...
export const podDownloadFile = (
  ...
  const transport = getDBXTransport()
  if (transport) {
    if (options?.isDirectory) return Promise.reject(new Error('目录下载尚不可用，请下载单个文件'))
    return downloadDBXFile((m,p)=>transport.rpc(m,p),{namespace,name:podName,container,path})
  }
  const params = new URLSearchParams({
    container,
    path,
  ...
  const suggestedName = options?.isDirectory ? `${baseName}.tar` : baseName
