...
describe('PodTerminalFileTree', () => {
  it('lists pod directories and lazily expands a directory', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'podFiles.enterDirectory:app' })
    )
  ...
  it('does not query files until pod and container are available', () => {
    ...
    render(
      <PodTerminalFileTree namespace="default" podName="" containerName="" />
    )

    expect(mockUsePodFiles).toHaveBeenLastCalledWith('default', '', '', '/', {
      enabled: false,
    })
  })
