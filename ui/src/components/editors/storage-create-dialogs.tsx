...
function AccessModeSelect({
  ...
  return (
    <Select
      value={value}
      onValueChange={(next) => onChange(next as PersistentVolumeAccessMode)}
    >
      <SelectTrigger>
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="ReadWriteOnce">
          {t('storageCreate.accessMode.rwo')}
        </SelectItem>
        <SelectItem value="ReadOnlyMany">
          {t('storageCreate.accessMode.rom')}
        </SelectItem>
        <SelectItem value="ReadWriteMany">
          {t('storageCreate.accessMode.rwm')}
        </SelectItem>
      </SelectContent>
    </Select>
  )
...
export function StorageClassCreateDialog({
  ...
  const [reclaimPolicy, setReclaimPolicy] = useState<'Delete' | 'Retain'>(
    'Delete'
  )
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const storageClass: StorageClass = {
        ...
        mountOptions:
          parsedMountOptions.length > 0 ? parsedMountOptions : undefined,
      }
      ...
      toast.success(
        t('storageCreate.storageClassSuccess', { name: trimmedName })
      )
      onOpenChange(false)
      onSuccess(created)
    } catch (error) {
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div className="space-y-2">
            ...
            <Select
              value={templateId}
              onValueChange={(value) =>
                applyTemplate(value as StorageClassTemplateId)
              }
            >
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={reclaimPolicy}
                onValueChange={(value) =>
                  setReclaimPolicy(value as 'Delete' | 'Retain')
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <label className="flex items-center gap-3 rounded-md border p-3">
              <Switch
                ...
                onCheckedChange={setAllowExpansion}
              />
              <span className="text-sm">
                {t('storageClasses.allowExpansion')}
              </span>
            </label>
            <label className="flex items-center gap-3 rounded-md border p-3">
              <Switch checked={isDefault} onCheckedChange={setIsDefault} />
              <span className="text-sm">
                {t('storageClasses.defaultClass')}
              </span>
            </label>
          </div>
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={isCreating || !name.trim() || !provisioner.trim()}
            >
    ...
    try {
      ...
      const pvc: PersistentVolumeClaim = {
        ...
        metadata: {
          ...
          labels:
            Object.keys(parsedLabels).length > 0 ? parsedLabels : undefined,
        },
        spec: {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {t('storageCreate.pvcDescription')}
          </DialogDescription>
        </DialogHeader>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                value={name}
                onChange={(event) => setName(event.target.value)}
                placeholder="data-web-0"
                autoFocus
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="pvc-namespace">
            ...
            <div className="space-y-2">
              <Label htmlFor="pvc-storage">
                {t('storageCreate.requestStorage')}
              </Label>
              <Input
                id="pvc-storage"
                value={storage}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={t('storageCreate.storageClassOptional')}
              />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={volumeMode}
                onValueChange={(value) =>
                  setVolumeMode(value as 'Filesystem' | 'Block')
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="pvc-volume-name">
                {t('storageCreate.boundVolumeOptional')}
              </Label>
              <Input
                id="pvc-volume-name"
                value={volumeName}
                onChange={(event) => setVolumeName(event.target.value)}
                placeholder="pv-data-web-0"
              />
            </div>
            <div className="space-y-2">
...
export function PVCreateDialog({
  ...
  const [reclaimPolicy, setReclaimPolicy] = useState<
    'Retain' | 'Delete' | 'Recycle'
  >('Retain')
  const [sourceType, setSourceType] = useState<'hostPath' | 'nfs' | 'local'>(
    'hostPath'
  )
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={reclaimPolicy}
                onValueChange={(value) =>
                  setReclaimPolicy(value as 'Retain' | 'Delete' | 'Recycle')
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <div className="space-y-4 rounded-md border p-4">
            <div className="space-y-2">
              ...
              <Select
                value={sourceType}
                onValueChange={(value) =>
                  setSourceType(value as 'hostPath' | 'nfs' | 'local')
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {sourceType === 'nfs' ? (
                <div className="space-y-2">
                  <Label htmlFor="pv-nfs-server">
                    {t('storageCreate.nfsServer')}
                  </Label>
                  <Input
                    id="pv-nfs-server"
                    value={nfsServer}
                    onChange={(event) => setNfsServer(event.target.value)}
                    placeholder="10.0.0.10"
                  />
                </div>
              ) : null}
              <div className="space-y-2">
                <Label htmlFor="pv-source-path">
                  {t('storageCreate.sourcePath')}
                </Label>
                <Input
                  id="pv-source-path"
                  value={path}
              ...
              {sourceType === 'local' ? (
                <div className="space-y-2">
                  <Label htmlFor="pv-node-hostname">
                    {t('storageCreate.nodeHostnameOptional')}
                  </Label>
                  <Input
                    id="pv-node-hostname"
                    value={nodeHostname}
                    onChange={(event) => setNodeHostname(event.target.value)}
