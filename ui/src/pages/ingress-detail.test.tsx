...
vi.mock('@/hooks/use-license', () => ({ useFeature: () => mockUseFeature() }))
...
describe('IngressDetail', () => {
  ...
  it('keeps history unavailable when its capability is disabled', () => {
    mockUseFeature.mockReturnValue(false)
    mockUseResource.mockReturnValue({ data: { metadata: { name: 'gateway', namespace: 'default', creationTimestamp: '2026-05-10T13:53:27.000Z' }, spec: {} }, isLoading: false })
    renderIngressDetail()
    expect(mockResourceHistoryTable).not.toHaveBeenCalled()
  ...
  it('edits ingress routes, tls, labels, and annotations by host group', async () => {
    ...
    expect(
      within(editDialog).getByText('ingresses.rulesTab')
    ).toBeInTheDocument()
    expect(
      within(editDialog).getByText('ingresses.certificatesTab')
    ).toBeInTheDocument()
    expect(
      within(editDialog).getByText('ingresses.metadataSettings')
    ).toBeInTheDocument()
    expect(
      within(editForm).getAllByDisplayValue('app.example.com').length
    ).toBeGreaterThan(0)
    ...
    const serviceInputs = within(editForm).getAllByLabelText(
      'ingresses.serviceName'
    )
    fireEvent.change(serviceInputs[0], { target: { value: 'web' } })
    const servicePortInputs = within(editForm).getAllByLabelText(
      'ingresses.servicePort'
    )
    ...
    await user.click(
      within(editForm).getByRole('tab', { name: 'ingresses.certificatesTab' })
    )
    expect(within(editForm).getByDisplayValue('app-tls')).toBeInTheDocument()

    await user.click(
      within(editForm).getByRole('tab', { name: 'ingresses.metadataSettings' })
    )
    ...
    const labelKeyInputs =
      within(editForm).getAllByLabelText('ingresses.labelKey')
    await user.type(labelKeyInputs[0], 'tier')
    const labelValueInputs = within(editForm).getAllByLabelText(
      'ingresses.labelValue'
    )
