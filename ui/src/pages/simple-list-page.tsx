...
export function SimpleListPage<T extends keyof ResourceTypeMap>({
  ...
  const [labelsResource, setLabelsResource] = useState<
    ResourceTypeMap[T] | null
  >(null)
  const [annotationsResource, setAnnotationsResource] = useState<
    ResourceTypeMap[T] | null
  >(null)
  ...
  const getRowContextMenuItems = useCallback(
    (
      resource: ResourceTypeMap[T]
    ): RowContextMenuItem<ResourceTypeMap[T]>[] => [
      {
        key: 'view-yaml',
