...
describe('storage resource details', () => {
  ...
  it('shows pvc binding, capacity, and consuming pods', () => {
    ...
    expect(
      screen.getByText('storageDetails.pvcInformation')
    ).toBeInTheDocument()
    ...
    expect(
      mockResponsiveTabs.mock.calls[0][0].map(
        (tab: { value: string }) => tab.value
      )
    ).toEqual(['overview', 'yaml', 'Related', 'events', 'history'])
  })
  ...
  it('shows pv binding, source type, and reclaim risk', () => {
    ...
    expect(
      screen.getByText('storageDetails.releasedRetainWarning')
    ).toBeInTheDocument()
    expect(screen.getByText('CSI')).toBeInTheDocument()
    expect(screen.getByText('ebs.csi.aws.com')).toBeInTheDocument()
    expect(
      screen.getByRole('link', { name: 'default/data-web-0' })
    ).toHaveAttribute('href', '/persistentvolumeclaims/default/data-web-0')
  })
  ...
  it('shows storage class policy and linked pvcs and pvs', () => {
    ...
    expect(
      screen.getByText('storageDetails.storageClassInformation')
    ).toBeInTheDocument()
    expect(screen.getByText('ebs.csi.aws.com')).toBeInTheDocument()
