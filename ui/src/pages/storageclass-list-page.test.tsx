...
describe('StorageClassListPage', () => {
  ...
  it('renders storage class operational columns', () => {
    ...
    expect(
      screen.getByRole('button', { name: 'storageClasses.viewParameters' })
    ).toHaveTextContent('2')
    expect(
      screen.getByRole('button', { name: 'storageClasses.viewMountOptions' })
    ).toHaveTextContent('1')
  })
  ...
  it('searches storage classes by provisioner, policy, mode, and parameters', () => {
    ...
    expect(resourceTableProps.searchQueryFilter(storageClass, 'delete')).toBe(
      true
    )
    expect(resourceTableProps.searchQueryFilter(storageClass, 'consumer')).toBe(
      true
    )
    expect(resourceTableProps.searchQueryFilter(storageClass, 'gp3')).toBe(true)
    expect(resourceTableProps.searchQueryFilter(storageClass, 'missing')).toBe(
      false
    )
  })
  ...
  it('provides unified row actions for storage classes', async () => {
    ...
    expect(mockNavigate).toHaveBeenCalledWith(
      '/storageclasses/fast-ssd?tab=yaml'
    )
