...
import { EndpointPort, Endpoints, Pod, Service } from 'kubernetes-types/core/v1'
import {
  EndpointSlice,
...
function getPodImages(pod: Pod) {
  const images = pod.spec?.containers?.map(
...
function getEndpointRows(
  ...
  const endpointRows = endpoints?.subsets?.flatMap((subset, subsetIndex) =>
    [
  ...
  return endpointSlices.flatMap((slice) =>
    (slice.endpoints || []).flatMap((endpoint, endpointIndex) =>
      (endpoint.addresses || []).map((address, addressIndex) => ({
        ...
        ports:
          (slice.ports || []).map(formatSliceEndpointPort).join(', ') || '-',
      }))
    )
  )
}
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex w-full flex-wrap gap-2 md:w-auto md:justify-end">
          ...
          <DescribeDialog
            ...
            name={name}
          />
          <DBXPortForwardButton resource="services" namespace={namespace || 'default'} name={name} />
          <Button
            variant="destructive"
            size="sm"
